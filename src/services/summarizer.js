 ...
 * @param {number} [hours=1] - Number of hours to summarize (default 1)
 * @returns {boolean} - Whether the summary was created and posted successfully
 */
export async function createSummary(client, hours = 1) {
  try {
    ...
    // Set the time period for messages (last N hours)
    const startTime = new Date();
    startTime.setHours(startTime.getHours() - hours);
    logger.info(
      `Summarizing messages from the last ${hours} hour(s), starting at ${startTime.toLocaleString()}`
    );
    ...
    if (totalMessages === 0) {
      logger.info(
        "No new messages found in monitored channels. Skipping summary."
      );
        ...
        if (summary) {
          ...
          const embed = new EmbedBuilder()
            ...
            .setFooter({
              text: `${
                messages.length
              } messages summarized • ${new Date().toLocaleString()}`,
            });
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a helpful assistant that summarizes Discord conversations accurately and concisely.",
        },
